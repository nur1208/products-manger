...
public class AddNewProductStage extends Stage implements DBInfo {
    ...
    Button chooseVideoButton = new Button("choose Video", new ImageView( new Image(getClass().getResourceAsStream("/images/add-image.png"))));
    ...
    public AddNewProductStage(ObservableList data){
        ...
        // Here we have defined properties of things to add in the popup window.

        chooseVideoButton.setFont(Font.font("Arial", FontWeight.BOLD, 15));
        ...
        descriptionTA.setStyle("-fx-border-color: lightgray; -fx-border-width: 2;");
        ...
        pane.getChildren().addAll(productImage,chooseImageButton,nameField, priceField, addButton,descriptionTA, descriptionLabel, productVideo, chooseVideoButton, quantityField );

        Play.setVisible(false);
        Stop.setVisible(false);
        Stop.setVisible(false);
        // set default input at name field
...
//            Integer.parseInt(quantityField.getText());
            ...
            alert.show("Error", "Price should be a decimal number (eg: 40, 10.5) and quantity should be integer number (eg: 40 )", Alert.AlertType.ERROR);
            return false;
        }
    }
    ...
    private void chooseImage(){
        ...
        fileChooser.getExtensionFilters().add(
                new FileChooser.ExtensionFilter("Select a .JPG .PNG .webp .GIF image", "*.jpg", "*.png", "*.webp","*.gif")
        );
        ...
        if (checkInputs()) {
            try {
                ...
                if (selectedImage == null && selectedVideo == null) {
                    ps = con.prepareStatement("INSERT INTO PRODUCTS(name, price, added_date, descr, quantity) values(?,?,?,?,?)");
                }
                // If the user puts an image of the product, the information he entered will be stored along with the image he selected as well
                    ...
                    ps = con.prepareStatement("INSERT INTO products(name, price, added_date, descr, quantity,image_url) values(?,?,?,?,?,?)");
...
//                    String createVideoPath = Common.saveSelectedImage(selectedVideo);
                    ...
                    ps = con.prepareStatement("INSERT INTO products(name, price, added_date, descr, quantity,image_url, videoUrl) values(?,?,?,?,?,?, ?)");
                    ps.setString(6, createImagePath);
    ...
    private  void viewProductInTheTable(){
        ...
        try{
            ...
            // it's necessary step to get the wanted information.
            rs.next();

            // The product data that will be returned each time will be temporarily stored in product object
